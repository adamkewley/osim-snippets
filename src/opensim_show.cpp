        ...
        while (true) {
            ...
            for (auto const& c : ms.cylinders) {
                ...
                auto hacky_cylinder_correction =
                        glm::rotate(glm::identity<glm::mat4>(), static_cast<float>(M_PI/2.0f), glm::vec3{0.0f, 0.0f, 0.0f});
...
namespace examples::imgui {
    ...
    struct FileChangeWatcher {
        ...
        FileChangeWatcher(std::string _path) :
            ...
            watcher_thread([&]() {
                for (;;) {
                    ...
                    std::string new_content = slurp_file(path);
        ...
        bool did_change() {
            bool expected = true;
            return changed.compare_exchange_weak(expected, false);
        }
    };
